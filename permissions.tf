...
resource "azurerm_role_assignment" "acr_pull" {
  count                            = var.enable && (var.os_type == "Linux" ? var.linux_app_stack.docker.enabled : var.windows_app_stack.docker.enabled) && var.site_config.container_registry_use_managed_identity == true ? 1 : 0
